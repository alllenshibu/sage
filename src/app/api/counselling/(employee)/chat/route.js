...
export async function GET(Request) {
    ...
    try {
        ...
        if (session.role === 'ROLE_EMPLOYEE') {
            ...
            // Database access
            const { rows } = await pool.query("SELECT chat_room_id FROM counselling_request LEFT JOIN counselling_session cs on counselling_request.id = cs.counselling_request_id WHERE employee_id = $1;", [session.uid]);
